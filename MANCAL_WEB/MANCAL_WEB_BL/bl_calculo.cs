...
namespace MANCAL_WEB_BL
{
    public class bl_calculo
    {
        ...
        public Cotizacion getMargenTotal(Cotizacion cotin, String iduneg)
        {
            ...
            DataTable dt = objCalculo.selectTotalDetalle(tipomoneda, cotin.cot_descuento, cotin.cot_dcto_porc, idniva, cotin.cot_id, iduneg).Tables["cur_ret_total"];
            ...
            foreach (DataRow dr in dt.Rows) 
            {
                ...
                if (cotin.cot_dcto_porc.Equals("0") & !cotin.cot_descuento.Equals("0"))
                {
                    cot.cot_dcto_porc = dr["DESCUENTO"].ToString();
                }
                else if (!cotin.cot_dcto_porc.Equals("0") & cotin.cot_descuento.Equals("0"))
                {
                    cot.cot_descuento = dr["DESCUENTO"].ToString();
                }
